...
public final class NetworkUtils {
    ...
    private static final String BASE_WEATHER_URI_LOC="https://api.openweathermap.org/data/2.5/weather";

    private static final String FORECAST_BASE_URL = BASE_WEATHER_URI_LAT_LON;
    ...
    public static URL getUrl(Context context) {
        ...
        if (SunshinePreferences.isLocationLatLonAvailable(context)) {
            ...
            Log.i(TAG+"###","Location coordinates are, latitude:"+latitude+"longitude: "+longitude);
    ...
    private static URL buildUrlWithLatitudeLongitude(Double latitude, Double longitude) {
       /* Uri weatherQueryUri = Uri.parse(FORECAST_BASE_URL).buildUpon()
        ...
        Uri weatherQueryUri=Uri.parse(BASE_WEATHER_URI_LAT_LON).buildUpon()
                .appendQueryParameter(LAT_PARAM, String.valueOf(latitude))
                .appendQueryParameter(LON_PARAM, String.valueOf(longitude))
                .appendQueryParameter(EXCLUDE_PARAM,"hourly")
                .appendQueryParameter(EXCLUDE_PARAM,"minutely")//Not working ??
                .appendQueryParameter(FORMAT_PARAM, format)
                .appendQueryParameter(UNITS_PARAM, units)
                .appendQueryParameter(DAYS_PARAM, Integer.toString(numDays))
                .appendQueryParameter(API_ID_param,"3a1494d2ae7ade33cca848d8fb66576f")
                .build();
...
//        Log.v(TAG+"###","Weather Query URL are:"+weatherURL);
...
//                .build();
        ...
        if(gc.isPresent()) {
            try {
                list = gc.getFromLocationName(locationQuery, 1);
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        return buildUrlWithLatitudeLongitude(lat,lng);
//        Uri weatherQueryUri=Uri.parse(BASE_WEATHER_URI_LOC).buildUpon()
//                .appendQueryParameter(QUERY_PARAM,locationQuery)
//                .appendQueryParameter(FORMAT_PARAM,format)
//                .appendQueryParameter(UNITS_PARAM,units)
//                .appendQueryParameter(DAYS_PARAM,Integer.toString(numDays))
...
//        }
    ...
    public static String getResponseFromHttpUrl(URL url) throws IOException {
        ...
        try {
            ...
            scanner.close();
            Log.i("NetworkUtils"+"aaaa","JSON RESOPNSE IS \n"+response);
            return response;
